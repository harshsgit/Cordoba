        ...
        .run([
            ...
            function ($templateCache) {
                $templateCache.put('template/treeGrid/treeGrid.html',
                    "<div class=\"table tree-grid table-responsive\">\n" +
                    " <table class=\"table grid table-hover  table-responsive tree-grid darkBackground\" id=\"tblAssociateGrid\" width=\"100%\"  >\n" +
                    "   <thead >\n" +
                    "     <tr width=\"100%\" >\n" +
                    "       <th  ng-repeat=\"col in colDefinitions\" ng-if=\"col.field=='InternalId'\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" data-sortedcolumn=\"{{col.field+' '+ col.sortDirection}}\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
                    "       <th ><a ng-if=\"expandingProperty.sortable\" ng-click=\"sortBy(expandingProperty)\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</a><span ng-if=\"!expandingProperty.sortable\">{{expandingProperty.displayName || expandingProperty.field || expandingProperty}}</span><i ng-if=\"expandingProperty.sorted\" data-sortedcolumn=\"{{expandingProperty.field+' '+ expandingProperty.sortDirection}}\"   class=\"{{expandingProperty.sortingIcon}} pull-right \"></i><br/><span >(<a ng-click='expand_all()' class='text-success cursor-pointer'>Show All</a>)</span></th>\n" +
                    "       <th ng-repeat=\"col in colDefinitions\" ng-if=\"col.field!='InternalId'\"><a ng-if=\"col.sortable\" ng-click=\"sortBy(col)\">{{col.displayName || col.field}}</a><span ng-if=\"!col.sortable\">{{col.displayName || col.field}}</span><i ng-if=\"col.sorted\" data-sortedcolumn=\"{{col.field+' '+ col.sortDirection}}\" class=\"{{col.sortingIcon}} pull-right\"></i></th>\n" +
                    "     </tr>\n" +
                    "   </thead>\n" +
                    "   <tbody>\n" +
                    "     <tr  ng-repeat=\"row in tree_rows | searchFor:$parent.filterString:expandingProperty:colDefinitions track by row.branch.uid\"\n" +
                    "       ng-class=\"row.branch.EmploymentStatusCd == 'T' || row.branch.EmploymentStatusCd == 'D' ? ' terminateAssociateInTree ':'' + 'level-' + {{ row.level }} + (row.branch.selected ? ' active':'')\"  class=\"tree-grid-row \">\n" +
                    "       <td ng-repeat=\"col in colDefinitions\" ng-if=\"col.field=='InternalId'\">\n" +
                    "         <div ng-if=\"col.cellTemplate\" compile=\"col.cellTemplate\" cell-template-scope=\"col.cellTemplateScope\"></div>\n" +
                    "         <div ng-if=\"!col.cellTemplate\">{{row.branch[col.field]}}</div>\n" +
                    "       </td>\n" +
                    "       <td><a ng-click=\"user_clicks_branch(row.branch)\"><i ng-class=\"row.tree_icon\"\n" +
                    "              ng-click=\"row.branch.expanded = !row.branch.expanded\"\n" +
                    "              class=\"indented tree-icon\" style=\"position:inherit\"></i></a><span ng-if=\"expandingProperty.cellTemplate\" class=\"indented tree-label InheritPosition\" " +
                    "              ng-click=\"on_user_click(row.branch)\" compile=\"expandingProperty.cellTemplate\" style=\"position:inherit\"></span>" +
                    "              <span  ng-if=\"!expandingProperty.cellTemplate\" class=\"indented tree-label \" ng-click=\"on_user_click(row.branch)\" style=\"position:inherit\">\n" +
            ...
            function ($timeout,
                ...
                return {
                    ...
                    link: function (scope, element, attrs) {
                        ...
                        attrs.iconExpand = attrs.iconExpand ? attrs.iconExpand : 'icon-plus  glyphicon glyphicon-plus  fa fa-plus IsParent';
                        attrs.iconCollapse = attrs.iconCollapse ? attrs.iconCollapse : 'icon-minus glyphicon glyphicon-minus fa fa-minus  IsParent';
                        attrs.iconLeaf = attrs.iconLeaf ? attrs.iconLeaf : 'icon-file  glyphicon glyphicon-file  fa fa-file IsLeaf';
                        ...
                        var formatToPercentage_RangePenetration = function (value, sliderId, label) {
                            switch (label) {
                                case 'model':
                                    return value + '%';
                                default:
                        ...
                        scope.RangePenetration = new Object();
                        scope.RangePenetration.value = 0;
                        scope.RangePenetration.options = {
                            floor: 0,
                            ceil: 100,
                            readOnly: true,
                            translate: formatToPercentage_RangePenetration,
                        ...
                        on_treeData_change = function () {
                            ...
                            //setTimeout(function () {
                            //    $(".IsLeaf").closest("tr").css("background-color", 'yellow');
                            //    $(".IsParent").closest("tr").css("background-color", 'pink');
                            //}, 0);
